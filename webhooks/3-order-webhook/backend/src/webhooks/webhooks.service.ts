...
export class WebhooksService {
  ...
  handleEvent(body: any, headers: Record<string, string>) {
    ...
    const orders = body.order_details || [];
    console.log(`[Webhook] orders.updated — ${orders.length} order(s), statuses: ${orders.map((o: any) => o.order_status).join(', ')} at ${event.receivedAt}`);
