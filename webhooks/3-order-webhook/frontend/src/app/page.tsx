...
export default function Page() {
  ...
  return (
    <div className="container">
      <h1>Order Status Webhook</h1>
      <p className="subtitle">
        Receives <code>orders.updated</code> events. Triggered when an order status changes or order details are modified.
      </p>
      ...
      <div className="card">
        ...
        <p style={{ fontSize: '0.875rem', color: '#718096', marginBottom: '16px' }}>
          ...
          <code>ngrok http 3042</code> — then paste the generated URL below.
        </p>
      ...
      <div className="card">
        ...
        {events.length === 0 ? (
          <div className="empty-state">
            No events yet. Register this server and update an order in Wizlo.
          </div>
        ) : (
          events.map(ev => (
            <div key={ev.id} style={{ border: '1px solid #e2e8f0', borderRadius: '6px', marginBottom: '10px', overflow: 'hidden' }}>
              ...
              >
                ...
                {ev.payload?.order_details?.[0] && (
                  <span style={{ fontSize: '0.8125rem', color: '#4a5568' }}>
                    {ev.payload.order_details[0].order_no} ·{' '}
                    <span className={`badge ${STATUS_COLORS[ev.payload.order_details[0].order_status] || 'badge-yellow'}`}>
                      {ev.payload.order_details[0].order_status}
                    </span>
                  </span>
                )}
