...
export class WebhooksService {
  ...
  handleEvent(body: any, headers: Record<string, string>) {
    ...
    console.log(`[Webhook] shipping.${event.eventType} — ${ship?.shipping_no || '?'} status: ${ship?.shipping_status || '?'} at ${event.receivedAt}`);
  ...
  async register(url: string, event = 'updated', secret?: string) {
    const config: any = {
      name: `Shipping ${event} Webhook`,
      module: 'shipping',
      event,
      url,
      maxRetries: 3,
