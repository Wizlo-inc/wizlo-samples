...
const EVENT_BADGE: Record<string, string> = {
  ...
  updated: 'badge-blue',
  deleted: 'badge-red',
  active_changed: 'badge-yellow',
  availability_changed: 'badge-purple',
  bulk_active_changed: 'badge-yellow',
  bulk_deleted: 'badge-red',
};
...
export default function Page() {
  ...
  function getProductSummary(ev: WebhookEvent) {
    const p = ev.payload;
    if (p.product) return `${p.product.name} (${p.product.sku})`;
    if (p.updatedProducts?.length) return `${p.updatedProducts.length} product(s) — isActive: ${p.isActive}`;
    if (p.productIds?.length) return `IDs: ${p.productIds.slice(0, 2).join(', ')}${p.productIds.length > 2 ? '…' : ''}`;
    return '';
  }

  return (
    <div className="container">
      <h1>Product Webhook</h1>
      <p className="subtitle">
        Receives <code>products.updated</code> events including: created, updated, deleted, active_changed,
        availability_changed, bulk_active_changed, bulk_deleted.
      </p>
      ...
      <div className="card">
        ...
        <p style={{ fontSize: '0.875rem', color: '#718096', marginBottom: '16px' }}>
          ...
          <code>ngrok http 3043</code> — then paste the generated URL below.
        </p>
      ...
      <div className="card">
        ...
        {events.length === 0 ? (
          <div className="empty-state">
            No events yet. Register this server and modify a product in Wizlo.
          </div>
        ) : (
          events.map(ev => (
            <div key={ev.id} style={{ border: '1px solid #e2e8f0', borderRadius: '6px', marginBottom: '10px', overflow: 'hidden' }}>
              ...
              >
                ...
                <span style={{ fontSize: '0.8125rem', color: '#4a5568' }}>{getProductSummary(ev)}</span>
