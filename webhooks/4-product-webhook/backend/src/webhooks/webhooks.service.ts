...
export class WebhooksService {
  ...
  handleEvent(body: any, headers: Record<string, string>) {
    ...
    const productName = body.product?.name || (body.updatedProducts?.[0]?.name) || body.productIds?.[0] || '?';
    console.log(`[Webhook] products.${event.eventType} — product: ${productName} at ${event.receivedAt}`);
  ...
  async register(url: string, _event?: string, secret?: string) {
    const config: any = {
