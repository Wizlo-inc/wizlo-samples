...
export class WebhooksService {
  ...
  handleEvent(body: any, headers: Record<string, string>) {
    ...
    const event: ReceivedEvent = {
      ...
      module: body.module || 'notification',
      channel: body.channel || 'unknown',
      payload: body,
    };
    this.events.unshift(event);
    if (this.events.length > 100) this.events.pop();
    const recipient = body.recipient?.email || body.recipient?.phone || '?';
    console.log(`[Webhook] notification.${event.eventType} — channel: ${event.channel} to: ${recipient} at ${event.receivedAt}`);
  ...
  async register(url: string, event = 'appointment', secret?: string) {
    const config: any = {
      name: `Notification ${event} Webhook`,
      module: 'notification',
      event,
      url,
      maxRetries: 3,
