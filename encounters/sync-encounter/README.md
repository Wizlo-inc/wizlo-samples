...
## Step-by-Step: How Encounter Creation Works
...
Open the frontend at `http://localhost:3011`.

![Create Encounter Form](screenshots/encounter-create.png)
...
**Form IDs** *(optional)*
Intake forms you want to attach to this encounter. Enter multiple IDs separated by commas.
...
Any free-text notes for the reviewer — e.g. `Patient requested morning slot`.
...
> Fields like `reviewType`, `serviceQueue`, `source`, and timezone are fixed defaults set by the backend — you do not need to pass them from the form.
...
**On validation error (400):** The backend returns a descriptive error — e.g. if `patientId` is missing or a field has the wrong type.
...
```
Browser (localhost:3011)
  └─ POST /encounters → Backend (localhost:3001)
