# HRT Encounter Sample

Demonstrates creating a Hormone Replacement Therapy (HRT) encounter in Wizlo using the Wizlo API.

## What This Sample Demonstrates
- OAuth2 client credentials authentication with Wizlo
- Creating an HRT encounter via `POST /encounters` with `isHrtTrtEncounter: true`
...
### POST /encounters
Creates an HRT encounter in Wizlo.

```bash
curl -X POST http://localhost:3003/encounters \
  -H "Content-Type: application/json" \
  -d '{
    "patientId": "49f623c9-0fc3-4e66-9b5e-56c955a71e43",
    "treatmentIds": ["treatment-uuid-1"],
    "additionalNotes": "Patient is on HRT program",
...
## Step-by-Step: How HRT Encounter Creation Works
...
Open the frontend at `http://localhost:3013`.

![HRT Encounter Form](screenshots/HRT-encounter.png)
...
HRT treatments to include in this encounter. Enter multiple IDs separated by commas.
Example: `treatment-1, treatment-2`

**Additional Notes** *(optional)*
Any free-text notes for the reviewer — e.g. `Patient is on HRT program`.
...
> `isHrtTrtEncounter: true` is what distinguishes this from a standard encounter. All other scheduling and queue fields are identical to a standard synchronous encounter.
...
```
Browser (localhost:3013)
  └─ POST /encounters → Backend (localhost:3003)
