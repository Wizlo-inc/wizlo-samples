...
interface MockForm { id: string; name: string; description?: string; status?: string; }
...
export default function IntakeFormPage() {
  ...
  const [selectedForm, setSelectedForm] = useState<MockForm | null>(null);
  ...
  const toggleH = (key: 'healthGoals' | 'medicalConditions', val: string) =>
    setHd(p => ({ ...p, [key]: p[key].includes(val) ? p[key].filter((x: string) => x !== val) : [...p[key], val] }));
  ...
  const loadForms = async () => {
    ...
    setLoading(true); setError('');
    try {
      const list = await getForms() as MockForm[];
      setForms(list);
      setFormsLoaded(true);
    } catch (err: unknown) {
  ...
  const handleSelectForm = async (form: MockForm) => {
    setLoading(true); setError('');
    try {
      ...
      <div className="container">
        ...
        {allIntakes && (
          <div className="card" style={{ marginTop: 16 }}>
            <h2>All Submissions ({allIntakes.length})</h2>
            <ul className="subs-list">
              {(allIntakes as Record<string, string>[]).map((item, i) => (
                <li key={item.id ?? i}>
                  <strong>Form:</strong> {item.formId} — <strong>Patient:</strong> {item.patientId} — {item.createdAt}
                </li>
              ))}
            </ul>
          </div>
    ...
    <div className="container">
      ...
      {step === 1 && (
        <div className="card">
          ...
          <p style={{ color: '#6b7280', fontSize: 14, marginTop: 0 }}>Data is stored in memory — restarts clear all records.</p>
