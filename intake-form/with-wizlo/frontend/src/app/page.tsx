...
import { createPatient, getForms, submitIntake } from '@/lib/api';
...
export default function IntakeFormPage() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const parts = fd.fullName.trim().split(' ');
      const patient = await createPatient({
        firstName: parts[0],
        lastName: parts.length > 1 ? parts.slice(1).join(' ') : parts[0],
        email: `${fd.fullName.toLowerCase().replace(/\s+/g, '.')}@example.com`,
      });
      const forms = await getForms();
      if (!forms.length) throw new Error('No published forms found in Wizlo');
      const structure = {
        pages: [{
          id: 'page_intake',
          rows: [
            { id: 'row_name', order: 0, fields: [{ name: 'full_name', label: 'Full Name', value: fd.fullName }] },
            { id: 'row_dob', order: 1, fields: [{ name: 'date_of_birth', label: 'Date of Birth', value: fd.dateOfBirth }] },
            { id: 'row_gender', order: 2, fields: [{ name: 'gender', label: 'Gender', value: fd.gender }] },
            { id: 'row_height', order: 3, fields: [{ name: 'height', label: 'Height', value: `${fd.heightFt}ft ${fd.heightIn}in` }] },
            { id: 'row_weight', order: 4, fields: [{ name: 'weight_lbs', label: 'Weight (lbs)', value: fd.weightLbs }] },
            { id: 'row_activity', order: 5, fields: [{ name: 'activity_level', label: 'Activity Level', value: fd.activityLevel }] },
            { id: 'row_goals', order: 6, fields: [{ name: 'health_goals', label: 'Health Goals', value: fd.healthGoals.join(', ') }] },
            { id: 'row_conditions', order: 7, fields: [{ name: 'medical_conditions', label: 'Medical Conditions', value: fd.medicalConditions.join(', ') }] },
            { id: 'row_meds', order: 8, fields: [{ name: 'current_medications', label: 'Current Medications', value: fd.currentMedications }] },
            { id: 'row_allergies', order: 9, fields: [{ name: 'allergies', label: 'Allergies', value: fd.allergies }] },
            { id: 'row_smoking', order: 10, fields: [{ name: 'smoking_status', label: 'Smoking Status', value: fd.smokingStatus }] },
            { id: 'row_alcohol', order: 11, fields: [{ name: 'alcohol_use', label: 'Alcohol Use', value: fd.alcoholUse }] },
          ],
        }],
      };
      const result = await submitIntake({ formId: forms[0].id, patientId: patient.id, structure });
  ...
  if (submitted) {
    return (
      <div className="container">
        ...
        {submitResult && <div className="card"><h2>API Response</h2><div className="result-box"><pre>{JSON.stringify(submitResult, null, 2)}</pre></div></div>}
      </div>
    );
  }
      ...
      {step === 4 && (
        <div className="card">
          ...
          <div className="result-box">
            {[['Name', fd.fullName], ['DOB', fd.dateOfBirth], ['Gender', fd.gender],
              ['Height', `${fd.heightFt}ft ${fd.heightIn}in`], ['Weight', `${fd.weightLbs} lbs`],
              ['Activity', fd.activityLevel], ['Goals', fd.healthGoals.join(', ') || 'None selected'],
              ['Conditions', fd.medicalConditions.join(', ') || 'None selected'],
